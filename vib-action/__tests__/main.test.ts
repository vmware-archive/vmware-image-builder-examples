...
describe("VIB", () => {
  ...
  it("Loads event configuration from the environment path", async () => {
    process.env.GITHUB_EVENT_PATH = path.join(root, "__tests__/github-event-path.json");
    let eventConfig = await loadEventConfig()
    expect(eventConfig["pull_request"]["head"]["repo"]["url"]).toBe("https://api.github.com/repos/mpermar/vib-action-test")
  })

  it("When event configuration exists SHA archive variable is set from its data", async () => {
    process.env.GITHUB_SHA = "aacf48f14ed73e4b368ab66abf4742b0e9afae54";
    process.env.GITHUB_REPOSITORY = "vmware/vib-action";
    process.env.GITHUB_EVENT_PATH = path.join(root, "__tests__/github-event-path.json"); // overseeds the previous two env vars
